...
var RawTransactions = function () {
  ...
  _createClass(RawTransactions, [{
    ...
    value: function change(rawtx, prevTxs, destination, fee) {
      return _axios2.default.get(this.restURL + 'rawTransactions/change/' + rawtx + '/' + prevTxs + '/' + destination + '/' + fee).then(function (response) {
    ...
    value: function input(rawtx, txid, n) {
      return _axios2.default.get(this.restURL + 'rawTransactions/input/' + rawtx + '/' + txid + '/' + n).then(function (response) {
        return response.data;
      }).catch(function (error) {
        return JSON.stringify(error.response.data.error.message);
      });
    ...
    value: function opReturn(rawtx, payload) {
      return _axios2.default.get(this.restURL + 'rawTransactions/opReturn/' + rawtx + '/' + payload).then(function (response) {
        return response.data;
      }).catch(function (error) {
        return JSON.stringify(error.response.data.error.message);
      });
    ...
    value: function reference(rawtx, destination, amount) {
      return _axios2.default.get(this.restURL + 'rawTransactions/reference/' + rawtx + '/' + destination + '/' + amount).then(function (response) {
        return response.data;
      }).catch(function (error) {
        return JSON.stringify(error.response.data.error.message);
      });
