...
var DataRetrieval = function () {
  ...
  _createClass(DataRetrieval, [{
    ...
    value: function listPendingTransactions(address) {
      var path = this.restURL + 'dataRetrieval/listPendingTransactions';
      if (address) {
    ...
    value: function listProperties() {
      return _axios2.default.get(this.restURL + 'dataRetrieval/listProperties').then(function (response) {
        return response.data;
      }).catch(function (error) {
        return JSON.stringify(error.response.data.error.message);
      });
