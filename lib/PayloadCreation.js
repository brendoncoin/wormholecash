...
var PayloadCreation = function () {
  ...
  _createClass(PayloadCreation, [{
    ...
    key: 'issuanceFixed',
    value: function issuanceFixed(ecosystem, propertyPricision, previousId, category, subcategory, name, url, data, amount) {
      return _axios2.default.post(this.restURL + 'payloadCreation/issuanceFixed/' + ecosystem + '/' + propertyPricision + '/' + previousId + '/' + category + '/' + subcategory + '/' + name + '/' + url + '/' + data + '/' + amount).then(function (response) {
        return response.data;
      }).catch(function (error) {
        return JSON.stringify(error.response.data.error.message);
      });
